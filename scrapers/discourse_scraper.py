...
DATE_FROM = datetime(2025, 1, 1)  # As requested
DATE_TO = datetime(2025, 4, 14)   # As requested
...
def login_and_save_auth(playwright):
    ...
    print("Please log in manually using Google. Press 'Resume' in Playwright bar.")
    page.pause()
...
def scrape_posts(playwright):
    ...
    while True:
        ...
        print(f"Fetching page {page_num} for category {CATEGORY_ID}...")
    ...
    for topic in all_topics:
        ...
        if DATE_FROM <= created_at <= DATE_TO:
            ...
            accepted_answer_id = topic_data.get("accepted_answer", {}).get("post_id", topic_data.get("accepted_answer_post_id"))
            reply_counter = {}
            for post in posts:
            ...
            for post in posts:
                post_created_at = parse_date(post["created_at"])
                if DATE_FROM <= post_created_at <= DATE_TO:
    ...
    with open("data/raw/discourse_posts.json", "w", encoding="utf-8") as f:
...
def main():
    with sync_playwright() as p:
        ...
        context = browser.new_context(storage_state=AUTH_STATE_FILE if os.path.exists(AUTH_STATE_FILE) else {})
        page = context.new_page()
        if not os.path.exists(AUTH_STATE_FILE) or not is_authenticated(page):
